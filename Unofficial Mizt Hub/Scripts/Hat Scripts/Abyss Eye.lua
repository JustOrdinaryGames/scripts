...
local ATTACKS = {"Abyssal Shriek - Z","Abyssal Dash - X","Shadow Cloak - C","Black Blast - V"--,
--"Nightmare Fall - Q"
}
...
function ApplyDamage(Humanoid,Damage)
	...
	flingTarget = Humanoid.Parent
    coroutine.wrap(function() wait(1) flingTarget = miztgetcharacter() end)()
end

function ApplyAoE(POSITION,RANGE,MINDMG,MAXDMG,FLING,INSTAKILL)
	for index, CHILD in pairs(workspace:GetDescendants()) do
		if CHILD.ClassName == "Model" and CHILD ~= Character and CHILD ~= game.Players.LocalPlayer.Character then
...
function HIT()
    print("a")
    local hitb = CreateParta(BLADE, 1, 1, "SmoothPlastic", BrickColor.Random())
    hitb.Anchored = true
    hitb.Transparency = 0
    hitb.Size = Vector3.new(1,1,1)
    hitb.CFrame = BLADE.CFrame
    MagniDamage(hitb, 8, 24, 30, 0, "Normal", 0)
    hitb:Destroy()
end
